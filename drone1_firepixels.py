...
wpX = []
wpY = []
segment_fire_pixels = {}  # clave: número de segmento, valor: píxeles máximos de fuego
...
def detect_fire(segment_idx, segment_seen_prev):
    ...
    if results and results[0].boxes is not None:
        ...
        # Asegurarnos de que haya máscaras
        ...
        best_center = None
        best_pixels = 0  # Máximo de píxeles de fuego en este frame para este segmento
        ...
        for det_idx, (box, cls_id, cf) in enumerate(zip(boxes, classes, confs)):
            if result.names[cls_id].lower() == 'fire':
                ...
                # Obtener máscara correspondiente a esta detección
                mask = masks[det_idx]
                mask_resized = cv2.resize(mask, (w, h))
                mask_bin = (mask_resized > 0.5).astype(np.uint8)
                ...
                # Guardar solo la detección con más píxeles en este frame
                if fire_pixels > best_pixels:
                    best_pixels = fire_pixels
                    best_center = (cx, cy)
        
        if best_center is not None:
            # Actualizar el máximo de píxeles para este segmento
...
def run_route():
    ...
    # Ordenar segmentos por cantidad de píxeles de fuego (de mayor a menor)
    ...
    # CSV ordenado por fuego más grande a más chico
